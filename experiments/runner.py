...
class ExperimentRunner:
    ...
    def _load_config(self, path: str) -> dict:
        ...
        # attach raw yaml and checksum and an immutable snapshot
        checksum = hashlib.sha256(raw.encode('utf8')).hexdigest()
        snapshot = deepcopy(cfg)
        self._validate_snapshot_serializable(snapshot)
        # return dict with special fields to be used downstream
        out = cfg or {}
        out['_config_yaml_raw'] = raw
        out['_config_checksum'] = checksum
        out['_config_snapshot'] = snapshot
        return out

    def _validate_snapshot_serializable(self, snapshot: dict) -> None:
        try:
            json.dumps(snapshot, default=str)
        except Exception as exc:
            raise ConfigValidationError(f"Config snapshot must be serializable: {exc}") from exc
