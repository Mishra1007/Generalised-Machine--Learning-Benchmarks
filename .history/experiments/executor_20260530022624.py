...
class ExperimentExecutor:
    ...
    def run(self, config: Dict[str, Any], out_root: str = 'results') -> Dict[str, Any]:
        ...
        # Deterministic mode: best-effort controls
        deterministic = bool(config.get('deterministic', False))
        if deterministic:
            # best-effort: set thread-controlling env vars to 1
        ...
        if isinstance(models_cfg, dict):
            ...
            for entry in models_cfg:
                if isinstance(entry, str):
                    if deterministic:
                        models_map[entry] = create_model(entry, random_state=seed)
                    else:
                        models_map[entry] = create_model(entry)
                elif isinstance(entry, dict):
                    name = entry.get('name') or entry.get('type')
        ...
        # 3) Run validation on training data (ensures fairness)
        # persist CV info by default unless explicitly disabled
        ...
        try:
            ...
            # fallback: try constructing a fresh FoldManager with same params
            try:
                ...
                folds_info = fm.get_all_fold_info(X_train, y_train)
            except Exception:
                folds_info = []
        ...
        env_info = metadata_mod.capture_environment()
        ...
        exp_meta = metadata_mod.ExperimentMetadata(
            ...
            random_seed=seed,
            config_snapshot=config.get('_config_snapshot') or config,
            python_version=env_info.get('python_version'),
            library_versions=env_info.get('library_versions')
        )

        saved = metrics_storage.save_experiment_results(dataset_name, model_summaries, out_root=out_root, config=config, seed=seed, metadata={**exp_meta.to_dict(), 'environment': env_info}, validation_results=results_map if save_predictions else None)
