...
class ExperimentExecutor:
    ...
    def run(self, config: Dict[str, Any], out_root: str = 'results') -> Dict[str, Any]:
        ...
        if isinstance(models_cfg, dict):
            ...
            for name, args in models_cfg.items():
                try:
                    ...
                    if config.get('deterministic', False) and 'random_state' not in params:
        ...
        framework_version = str(config.get('framework_version') or metadata_mod.capture_library_versions(['pip']).get('pip', 'unknown'))
