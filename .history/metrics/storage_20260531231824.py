...
def _build_score_matrix(validation_results: Dict[str, Any], primary_metric: str = 'accuracy'):
    ...
    for model_name, vres in validation_results.items():
        ...
        for fold in fold_results:
            ...
            if primary_metric in metrics:
                scores.append(metrics[primary_metric])
        if scores:
    ...
    min_len = min(len(scores) for scores in score_rows)
    if min_len == 0:
        return None, model_names

    matrix = []
    for scores in score_rows:
        matrix.append(scores[:min_len])
        ...
        try:
            ...
            if matrix is not None and len(model_names) >= 2:
                ...
                for _, row in pairwise.iterrows():
                    ...
                    effect = effect_size_summary(np.asarray(matrix[idx_a], dtype=float), np.asarray(matrix[idx_b], dtype=float))
