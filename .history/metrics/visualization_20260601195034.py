...
def generate_plots_for_dataset(dataset_name: str, models_summaries: Dict[str, Dict[str, Any]], out_dir: str = 'plots') -> Dict[str, Tuple[str, str]]:
    ...
    """
    if not models_summaries:
        logger.warning(f"No model summaries available for dataset '{dataset_name}'. Skipping plot generation.")
        return {}
    ...
    if not model_names:
        logger.warning(f"No model names available for dataset '{dataset_name}'. Skipping plot generation.")
        return {}
    ...
    if normalized:
        ...
        logger.warning(f"No normalized metrics available for dataset '{dataset_name}'. Skipping heatmap.")
