...
def save_experiment_results(dataset_name: str, models_summaries: Dict[str, Dict[str, Any]], out_root: str = 'results', config: Dict[str, Any] = None, seed: int = None, metadata: Dict[str, Any] = None, validation_results: Dict[str, Any] = None) -> Dict[str, str]:
    ...
    logger.log('INFO', 'Experiment save complete', saved_files={'raw': raw_path, 'normalized': norm_path, 'cbs': cbs_path, 'config': cfg_path, 'manifest': manifest_path, 'report': report_path})
    logger.close()

    return {'raw': raw_path, 'normalized': norm_path, 'cbs': cbs_path, 'config': cfg_path, 'manifest': manifest_path, 'report': report_path, 'logs': os.path.join(base, LOG_FILENAME), 'plots_dir': os.path.join(out_root, dataset_name)}
