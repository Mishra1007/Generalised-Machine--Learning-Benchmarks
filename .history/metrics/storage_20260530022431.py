...
def save_experiment_results(dataset_name: str, models_summaries: Dict[str, Dict[str, Any]], out_root: str = 'results', config: Dict[str, Any] = None, seed: int = None, metadata: Dict[str, Any] = None, validation_results: Dict[str, Any] = None) -> Dict[str, str]:
    ...
    if metadata is not None:
        try:
            # Basic schema validation - fail fast if critical provenance missing
            required = ['experiment_id', 'timestamp', 'dataset_name', 'config_snapshot', 'random_seed']
    ...
    try:
        ...
        with open(metrics_path, 'w', encoding='utf8') as fh:
            json.dump(models_summaries, fh, indent=2, default=str)
