...
def test_deterministic_mode_identical_runs(n=10):
    ...
    try:
        ...
        cfg_path = tmp / 'deterministic.yaml'
        cfg_path.write_text(json.dumps(cfg), encoding='utf8')

        for i in range(n):
            out = tmp / f'res_{i}'
            out.mkdir(parents=True, exist_ok=True)
            wrapper = tmp / f'run_launcher_{i}.py'
            wrapper.write_text(
                f"""
import sys
from pathlib import Path

PROJECT_ROOT = Path(r'{Path(__file__).resolve().parents[1]}')
...
apply_early_binding(seed=42)
register_dataset('det_ds', r'{csv}', target_column='target')
sys.argv = ['launcher.py', '--config', r'{cfg_path}', '--out', r'{out}']
launcher_main()
""",
                encoding='utf8',
            )
            subprocess.run([sys.executable, str(wrapper)], cwd=str(Path(__file__).resolve().parents[1]), check=True, capture_output=True, text=True)
            base = out / 'det_ds'
            meta = json.loads((base / 'metadata.json').read_text())
            stable_meta = {k: v for k, v in meta.items() if k not in {'experiment_id', 'run_id', 'timestamp'}}
            ...
            outputs.append({
                ...
                'metrics': json.dumps(strip_time_fields(metrics), sort_keys=True, default=str),
