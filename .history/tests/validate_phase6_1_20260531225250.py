...
def test_deterministic_mode_identical_runs(n=10):
    ...
    try:
        ...
        for i in range(n):
            ...
            subprocess.run([sys.executable, 'launcher.py', '--config', str(cfg_path), '--out', str(out)], cwd=str(Path(__file__).resolve().parents[1]), check=True, capture_output=True, text=True)
            base = out / 'det_ds'
